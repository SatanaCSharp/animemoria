...
# Migration undo script that uses Node.js --env-file flag
# Runs the undo-migration script from @packages/nest-shared
...
echo "Undoing last migration with env file: $ENV_FILE"
...
if [ "$MODE" = "prod" ]; then
  ...
  node --env-file="$ENV_FILE" ./node_modules/@packages/nest-shared/dist/orm/migration/undo-migration.js
else
  ...
  node --env-file="$ENV_FILE" \
    ...
    ./node_modules/@packages/nest-shared/src/orm/migration/undo-migration.ts
fi
