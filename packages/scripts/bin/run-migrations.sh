...
# Migration runner script that uses Node.js --env-file flag
# Runs the nest-run-migrations binary from @packages/nest-shared
...
if [ "$MODE" = "prod" ]; then
  ...
  node --env-file="$ENV_FILE" ./node_modules/@packages/nest-shared/dist/orm/migration/run-migrations.js
else
  ...
  node --env-file="$ENV_FILE" \
    ...
    ./node_modules/@packages/nest-shared/src/orm/migration/run-migrations.ts
fi
